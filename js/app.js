...
function mostraWizardSetup() {
  ...
  wizard.innerHTML = `
    <div style="text-align:center;max-width:500px;width:100%;background:rgba(255,255,255,0.05);backdrop-filter:blur(10px);border:3px solid rgba(78,236,163,0.3);border-radius:24px;padding:40px 30px;box-shadow:0 20px 60px rgba(0,0,0,0.6)">
      ...
      <p style="font-size:1em;color:#d1d4dc;margin-bottom:35px">Configura Budget Manager in 3 semplici passaggi</p>
      ...
      <div id="wizardSteps">
        ...
        <div id="step3" class="wizard-step" style="display:none">
          ...
          <div id="ticketOptions" style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:25px">
          ...
          <button style="width:100%;padding:15px;background:linear-gradient(135deg,#4ecca3,#3cb371);border:none;border-radius:12px;color:#fff;font-weight:800;font-size:1.1em;cursor:pointer" onclick="completaSetup()">✓ Inizia</button>
        </div>
      </div>
    </div>
  `;
...
function completaSetup() {
  ...
  // NUOVO - Se modalità solo + nessun buono → rimuovi buoni dal DB
  if (wizardMode === 'solo' && !wizardTicketUser) {
    DB.conti.buoni_pasto = null;
    salvaDB();
  }
  
  // NUOVO - Nascondi card impostazioni buoni se nessuno ha buoni
  ...
  mostraToast('✅ Setup completato!', 'success');
  
  // Aggiorna UI
  aggiorna();
  aggiornaVisibilitaCampiModalita(); // NUOVO: Aggiorna visibilità nav e campi
}
...
function aggiornaListaObiettiviModal() {
  ...
  if (!DB.obiettivi || DB.obiettivi.length === 0) {
    container.innerHTML = '<div style="text-align:center;padding:30px;color:#7f8c8d">Nessun obiettivo impostato</div>';
    return;
  }
  ...
  DB.obiettivi.forEach(function(obj, index) {
    ...
    html += '<div style="background:var(--card);padding:15px;border-radius:10px;margin-bottom:10px;border-left:4px solid #ff9800">';
    html += '<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px">';
    html += '<div style="font-weight:700;font-size:1em">' + obj.icona + ' ' + obj.nome + '</div>';
    html += '<button onclick="eliminaObiettivo(' + index + ')" style="background:#e74c3c;color:white;border:none;padding:6px 12px;border-radius:6px;cursor:pointer;font-size:0.85em">🗑️ Elimina</button>';
    html += '</div>';
    html += '<div style="font-size:0.9em;color:#7f8c8d;margin-bottom:4px">Target: ' + formatEuro(obj.target) + ' • Risparmiato: ' + formatEuro(obj.risparmiato) + ' (' + percentuale + '%)</div>';
    html += '<button onclick="modificaRisparmiato(' + index + ')" style="width:100%;background:#3498db;color:white;border:none;padding:8px;border-radius:6px;cursor:pointer;font-size:0.9em;margin-top:8px">✏️ Modifica Importo Risparmiato</button>';
    html += '</div>';
  });
...
function salvaObiettivo() {
  ...
  if (!nome) {
    alert('Inserisci un nome per l\'obiettivo');
    return;
  }
...
function calcolaSaldiConti() {
  ...
  // Inizializza saldi a 0 per tutti i conti dinamici
  var saldi = {
    buoni_pasto: 0 // numero buoni
  };
