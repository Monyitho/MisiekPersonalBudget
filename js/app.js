...
function aggiornaWidgetObiettivi() {
  ...
  DB.obiettivi.slice(0, 3).forEach(function(obj) {
    // Risparmio = valore manuale iniziale + risparmi calcolati dalle transazioni
    ...
    // SOMMA il valore manuale iniziale ai risparmi dalle transazioni
    var risparmiato = risparmioIniziale + Math.max(0, risparmioCalcolato);
...
function aggiornaListaObiettiviInvestimento() {
  ...
  // Aggiorna totale risparmiato (manuale + calcolato)
  var totaleRisparmiato = DB.obiettivi.reduce(function(sum, obj) {
    ...
    return sum + risparmioIniziale + Math.max(0, risparmioCalcolato);
  }, 0);
