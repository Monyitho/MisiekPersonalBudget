    ...
    <div style="text-align:center;max-width:500px;width:100%;background:rgba(255,255,255,0.05);backdrop-filter:blur(10px);border:3px solid rgba(78,236,163,0.3);border-radius:24px;padding:40px 30px;box-shadow:0 20px 60px rgba(0,0,0,0.6);max-height:90vh;overflow-y:auto">
      ...
      <div id="wizardSteps">
        ...
        <div id="step2b" class="wizard-step" style="display:none">
          <div style="font-size:1.2em;font-weight:800;color:#e5e4e2;margin-bottom:15px;text-align:left">💰 Debito Iniziale</div>
          <div style="font-size:0.85em;color:#999;margin-bottom:20px;text-align:left;line-height:1.5">C'è un debito pregresso tra voi? Imposta chi deve a chi e quanto</div>
          ...
          <div style="background:rgba(231,76,60,0.1);border:2px solid rgba(231,76,60,0.3);border-radius:14px;padding:20px;margin-bottom:20px">
            <div style="display:flex;flex-direction:column;gap:15px">
              <!-- Chi deve -->
              <div>
                <label style="font-size:0.85em;font-weight:700;color:#e74c3c;margin-bottom:8px;display:block">Chi deve saldare?</label>
                <select id="wizardDebitore" style="width:100%;padding:12px;background:rgba(0,0,0,0.4);border:2px solid rgba(231,76,60,0.4);border-radius:10px;color:#fff;font-size:1em;font-family:inherit">
              ...
              <!-- Importo debito -->
              <div id="wizardDebitoImportoGroup" style="display:none">
                <label style="font-size:0.85em;font-weight:700;color:#e74c3c;margin-bottom:8px;display:block">Importo da saldare</label>
                <div style="display:flex;align-items:center;gap:10px">
                  <span style="font-size:1.3em;color:#e74c3c;font-weight:800">€</span>
                  <input type="number" id="wizardDebitoImporto" value="0" min="0" step="0.01" style="flex:1;padding:12px;background:rgba(0,0,0,0.4);border:2px solid rgba(231,76,60,0.4);border-radius:10px;color:#fff;font-size:1.2em;font-weight:700;font-family:inherit">
                </div>
              </div>
            </div>
          </div>
...
function completaSetup() {
  ...
  // Salva debito iniziale se in coppia
  if (wizardMode === 'couple' && wizardDebitoIniziale.debitore && wizardDebitoIniziale.importo > 0) {
    if (!DB.condiviso) DB.condiviso = {};
    // Se user deve, il partner deve recuperare (daRecuperare > 0)
    // Se partner deve, tu devi recuperare (daRecuperare < 0 o saldato a te)
    if (wizardDebitoIniziale.debitore === 'user') {
      // Tu devi saldare al partner
      DB.condiviso.debitoIniziale = -wizardDebitoIniziale.importo;
    } else {
      // Partner deve saldare a te
      DB.condiviso.debitoIniziale = wizardDebitoIniziale.importo;
    }
  }
...
function aggiornaListaObiettiviInvestimento() {
  ...
  DB.obiettivi.forEach(function(obj, index) {
    var percentuale = obj.target > 0 ? ((obj.risparmiato / obj.target) * 100) : 0;
    var mancante = Math.max(0, obj.target - obj.risparmiato);
    var color = colors[index % colors.length];
    ...
    html += '<div style="font-size:0.95em;font-weight:800;color:#2e7d32;margin-top:2px">' + formatEuro(obj.risparmiato) + '</div>';
    html += '</div>';
