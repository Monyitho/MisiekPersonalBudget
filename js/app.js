...
function inizializzaDragDropConti() {
  ...
  items.forEach(function(item) {
    ...
    item.addEventListener('touchstart', function(e) {
      // Controlla se il touch è sul drag handle (i puntini)
      var touch = e.touches[0];
      var target = document.elementFromPoint(touch.clientX, touch.clientY);

      // Verifica se stiamo toccando l'area del drag handle (primi 40px a sinistra)
      var rect = this.getBoundingClientRect();
      if (touch.clientX - rect.left > 50) {
        // Non è sul drag handle, lascia il comportamento normale
        return;
      }
    ...
    item.addEventListener('touchend', function(e) {
      ...
      // Ripristina stili dell'elemento trascinato
      contoTrascinato.style.opacity = '1';
    ...
    item.addEventListener('touchcancel', function(e) {
      // Ripristina tutto in caso di annullamento
      if (contoTrascinato) {
        contoTrascinato.style.opacity = '1';
        contoTrascinato.style.transform = '';
        contoTrascinato.style.zIndex = '';
