...
function aggiornaListaObiettiviModal() {
  ...
  DB.obiettivi.forEach(function(obj, index) {
    ...
    html += '<div style="background:var(--card);padding:15px;border-radius:12px;margin-bottom:12px;border:2px solid ' + progressColor + '">';

    // Header centrato con icona grande
    html += '<div style="text-align:center;margin-bottom:12px">';
    html += '<div style="font-size:2em;margin-bottom:6px">' + obj.icona + '</div>';
    html += '<div style="font-weight:700;font-size:1.1em;color:var(--text)">' + obj.nome + '</div>';
    html += '</div>';

    // Progress info centrato
    html += '<div style="text-align:center;font-size:0.85em;color:#7f8c8d;margin-bottom:10px">';
    html += '<div>Target: <strong>' + formatEuro(obj.target) + '</strong></div>';
    html += '<div>Risparmiato: <strong style="color:' + progressColor + '">' + formatEuro(risparmiato) + '</strong> (' + percentuale + '%)</div>';
    html += '</div>';

    // Barra progresso
    html += '<div style="background:var(--border);height:10px;border-radius:5px;overflow:hidden;margin-bottom:12px">';
    html += '<div style="background:' + progressColor + ';height:100%;width:' + Math.min(percentuale, 100) + '%;transition:width 0.3s"></div>';
    html += '</div>';

    // Date centrate e su righe separate
    if (obj.dataInizio || obj.dataFine) {
      html += '<div style="text-align:center;font-size:0.8em;color:#a1a1aa;margin-bottom:12px">';
      if (obj.dataInizio) {
        var dInizio = new Date(obj.dataInizio);
        html += '<div>📅 Inizio: ' + dInizio.getDate() + ' ' + mesiNomi[dInizio.getMonth()] + ' ' + dInizio.getFullYear() + '</div>';
      }
      if (obj.dataFine) {
        var dFine = new Date(obj.dataFine);
        html += '<div>🏁 Fine: ' + dFine.getDate() + ' ' + mesiNomi[dFine.getMonth()] + ' ' + dFine.getFullYear() + '</div>';
      }
      html += '</div>';
    }
    ...
    html += '<button onclick="modificaRisparmiato(' + index + ')" style="flex:1;background:linear-gradient(135deg,#3498db,#2980b9);color:white;border:none;padding:10px;border-radius:8px;cursor:pointer;font-size:0.85em;font-weight:600">✏️ Modifica</button>';
    html += '<button onclick="eliminaObiettivo(' + index + ')" style="background:#e74c3c;color:white;border:none;padding:10px 14px;border-radius:8px;cursor:pointer;font-size:0.85em">🗑️</button>';
    html += '</div>';
...
function apriModalAggiuntaConto() {
  ...
  document.getElementById('contoSaldoIniziale').value = '0';
...
function apriModalModificaConto(id) {
  ...
  document.getElementById('contoSaldoIniziale').value = saldoIniziale;
...
function salvaConto(e) {
  ...
  var saldoIniziale = parseFloat(document.getElementById('contoSaldoIniziale').value) || 0;
